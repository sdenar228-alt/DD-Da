...
**Ошибка:**
Клиент на маке по-прежнему открывался не на весь экран — уже после исправления
ошибки №7 (сообщение пользователя и лог с Apple M5, macOS 25.6, 2 сентября).

**Причина:**
Ошибка №7 была настоящей, но не той. Она чинила режим 1, а на macOS режим 1 не
стоит по умолчанию: в `config_variables.h` для `CONF_PLATFORM_MACOS` апстрим
DDNet задаёт **свои** умолчания — `gfx_borderless 1` и `gfx_fullscreen 0`. То
есть Mac-клиент по умолчанию вообще не полноэкранный: это безрамочное окно,
которое заполняет место между строкой меню и доком и выглядит как неудавшийся
полный экран. Именно это на фото.

**Исправление:**
Умолчание `gfx_fullscreen` для macOS — 2 (desktop fullscreen): своё
пространство, и то же самое, во что macOS всё равно превращает режим 1. Плюс
`backend_sdl.cpp` пишет в лог, чем окно стало на самом деле — «desktop
fullscreen» / «borderless window» / размер, — чтобы такой вопрос впредь решался
одним взглядом в лог, а не догадками.

Строка сначала попала не туда: якорь, по которому её вставляли, встречается в
файле дважды, и первое совпадение оказалось в headless-ветке — до того, как окно
вообще создано. Собиралось и молчало. Перенесена сразу за успешный
`SDL_CreateWindow` и проверена запуском: `Window is windowed, 800x500
(gfx_fullscreen 0, gfx_borderless 0, desktop 1920x1080)`.

Умолчание действует только на конфиг, где значения ещё нет. Тем, у кого клиент
уже запускался, значение записано в `settings_ddnet.cfg`, и его нужно поменять
один раз: `gfx_fullscreen 2` в консоли (F1) или Настройки → Графика.

**Проверка:**
Windows: `build.bat` зелёный, поведение Windows не изменилось — правка внутри
`#else` для macOS. macOS: сборка в CI; новая строка лога подтвердит режим с
первого же запуска у пользователя.
